...
if ($_POST) {
    ...
    if (isset($_GET['action']) && $_GET['action'] == 'modification') {
        ...
        $pdo->query("UPDATE orders 
                        SET id_user = '$_POST[id_user]', order_amount = '$_POST[order_amount]', order_date = '$_POST[order_date]', order_status = '$_POST[order_status]'
                        WHERE id_order = '$_POST[id_order]'");

        header('location:gestion_commande.php'); // je renvoi l'user sur le même fichier pour ne pas avoir infos du produit qui vient d'être update par défaut dans l'url
        ...
        $pdo->query("INSERT INTO orders (id_order, order_amount, order_date, order_status) VALUES ('$_POST[id_order]','$_POST[order_amount]', '$_POST[order_date]', '$_POST[order_status]')");
    }
}
...
if (isset($_GET['action']) && $_GET['action'] == 'suppression') {
    ...
    header('location:gestion_commande.php'); // je renvoi l'user sur le même fichier pour ne pas avoir infos du produit qui vient d'être delete par défaut dans l'url
}
...
if($r->rowCount() >1){ // Si j'ai plus de 1 produit dans ma boutique j'affiche Liste des n produits
    $content .= '<h1 class="text-center display-4">Liste des ' . $r->rowCount() . ' commandes de la boutique</h1>';
}else{ //Sinon j'affiche Liste de 1 produit
    $content .= '<h1 class="text-center display-4">Liste de ' . $r->rowCount() . ' commande de la boutique</h1>';
}
...
while ($row = $r->fetch(PDO::FETCH_ASSOC)) {
    ...
    $content .= "<td class=\"align-middle\"><a href=?action=modification&id_order=$row[id_order]>update</a></td>";
    $content .= "<td class=\"align-middle\"><a href=?action=suppression&id_order=$row[id_order]>Delete</a></td>";
...
if (isset($_GET['action']) && $_GET['action'] == 'modification') {
    ...
    $actual_orders = $r->fetch(PDO::FETCH_ASSOC);
}
...
$id_order = (isset($actual_orders['id_order'])) ? $actual_orders['id_order'] : '';
$id_user = (isset($actual_orders['id_user'])) ? $actual_orders['id_user'] : '';
$order_amount = (isset($actual_orders['order_amount'])) ? $actual_orders['order_amount'] : '';
$order_date = (isset($actual_orders['order_date'])) ? $actual_orders['order_date'] : '';
$order_status = (isset($actual_orders['order_status'])) ? $actual_orders['order_status'] : '';
...
<?php if (isset($_GET['action']) && $_GET['action'] == 'modification') : ?>
    <h4 class="text-center display-4  text-warning">Modification d'une commande </h4>
<?php else : ?>
    <h4 class="text-center display-4">Ajouter une commande</h4>
<?php endif ?>
...
<form method="post" action="" enctype="multipart/form-data">
    ...
    <input type="text" name="order_amount" placeholder="montant" id="order_amount" class="form-control" value="<?= $order_amount ?>"><br>

    <label for="order_date">Date de la commande</label>
    <input type="date" name="order_date" placeholder="Date de la commande" id="order_date" class="form-control" value="<?= $order_date ?>"><br>

    <label for="order_status">statut de la commande</label>
    <input type="text" name="order_status" placeholder="statut de la commande" id="order_status" class="form-control" value="<?= $order_status ?>"><br>
    ...
    <div class="text-center mb-5">
        ...
        <?php else : ?>
            <br><input type="submit" value="Ajouter la commande
