...
if ($_POST) {
    ...
    if (isset($_GET['action']) && $_GET['action'] == 'modification') {
        ...
        $pdo->query("UPDATE users 
                        SET firstname = '$_POST[firstname]', lastname = '$_POST[lastname]', email = '$_POST[email]', date_of_birth = '$_POST[date_of_birth]', password = '$_POST[password]'
                        WHERE id_user = '$_POST[id_user]'");

        header('location:gestion_membres.php'); // je renvoi l'user sur le même fichier pour ne pas avoir infos du produit qui vient d'être update par défaut dans l'url
        ...
        $pdo->query("INSERT INTO users (firstname, lastname, email, date_of_birth, password) VALUES ('$_POST[firstname]', '$_POST[lastname]', '$_POST[email]', '$_POST[date_of_birth]', '$_POST[password]')");
    }
}
...
if (isset($_GET['action']) && $_GET['action'] == 'suppression') {
    ...
    header('location:gestion_membres.php'); // je renvoi l'user sur le même fichier pour ne pas avoir infos du produit qui vient d'être delete par défaut dans l'url
}
...
if($r->rowCount() >1){ // Si j'ai plus de 1 produit dans ma boutique j'affiche Liste des n produits
    $content .= '<h1 class="text-center display-4">Liste des ' . $r->rowCount() . ' membres de la boutique</h1>';
}else{ //Sinon j'affiche Liste de 1 produit
    $content .= '<h1 class="text-center display-4">Liste de ' . $r->rowCount() . ' membre de la boutique</h1>';
}
...
while ($row = $r->fetch(PDO::FETCH_ASSOC)) {
    ...
    $content .= "<td class=\"align-middle\"><a href=?action=modification&id_user=$row[id_user]>update</a></td>";
    $content .= "<td class=\"align-middle\"><a href=?action=suppression&id_user=$row[id_user]>Delete</a></td>";
...
if (isset($_GET['action']) && $_GET['action'] == 'modification') {
    ...
    $actual_members = $r->fetch(PDO::FETCH_ASSOC);
}
...
$id_user = (isset($actual_members['id_user'])) ? $actual_members['id_user'] : '';
$firstname = (isset($actual_members['firstname'])) ? $actual_members['firstname'] : '';
$lastname = (isset($actual_members['lastname'])) ? $actual_members['lastname'] : '';
$email = (isset($actual_members['email'])) ? $actual_members['email'] : '';
$date_of_birth = (isset($actual_members['date_of_birth'])) ? $actual_members['date_of_birth'] : '';
$password = (isset($actual_members['password'])) ? $actual_members['password'] : '';
...
<?php if (isset($_GET['action']) && $_GET['action'] == 'modification') : ?>
    <h4 class="text-center display-4  text-warning">Modification d'un membre</h4>
<?php else : ?>
    <h4 class="text-center display-4">Ajouter un membre</h4>
<?php endif ?>
...
<form method="post" action="" enctype="multipart/form-data">
    ...
    <input type="text" name="firstname" placeholder="prenom du membre" id="firstname" class="form-control" value="<?= $firstname ?>"><br>

    <label for="lastname">Nom</label>
    <input type="text" name="lastname" placeholder="Nom du membre" id="lastname" class="form-control" value="<?= $lastname ?>"><br>

    <label for="email">Email</label>
    <input type="text" name="email" placeholder="email du membre" id="email" class="form-control" value="<?= $email ?>"><br>

    <label for="date_of_birth">Date de naissance</label>
    <input type="date" name="date_of_birth" placeholder="date de naissance du membre" id="date_of_birth" class="form-control"><?= $date_of_birth ?></input><br>
    ...
    <div class="text-center mb-5">
        ...
        <?php else : ?>
            <br><input type="submit" value="Ajouter le membre" class="btn btn-lg btn-primary">
        <?php endif ?>
    </div>
